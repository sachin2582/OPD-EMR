...
console.log('🔍 Checking opd-emr.db for dose_pattern table...');
...
const checkDosePatternTable = () => {
  ...
  db.get(checkTableSQL, [], (err, row) => {
    ...
    if (!row) {
      console.log('❌ dose_pattern table does not exist in opd-emr.db!');
      console.log('🔧 Creating dose_pattern table...');
      createDosePatternTable();
      return;
    }
    
    console.log('✅ dose_pattern table exists in opd-emr.db');
  ...
  dosePatterns.forEach((pattern, index) => {
    db.run(insertSQL, [
      ...
      if (completed === dosePatterns.length) {
        console.log('✅ All dose patterns inserted successfully into opd-emr.db');
