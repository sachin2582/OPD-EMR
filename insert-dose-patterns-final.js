...
// Database path - the actual database file the backend uses
const dbPath = path.join(__dirname, 'backend', 'opd-emr.db');

console.log('🚀 Inserting dose patterns into opd-emr.db...');
...
const db = new sqlite3.Database(dbPath, (err) => {
  ...
  console.log('✅ Connected to opd-emr.db');
...
const insertDosePatterns = () => {
  ...
  db.run(createTableSQL, (err) => {
    ...
    console.log('✅ dose_pattern table created/verified');

    // Clear existing data
    db.run('DELETE FROM dose_pattern', (err) => {
      if (err) {
            ...
            db.get(countSQL, [], (err, result) => {
              ...
              db.close((err) => {
                if (err) {
                  ...
                  console.log('🎉 Dose patterns successfully inserted into opd-emr.db!');
                }
              });
            });
          }
