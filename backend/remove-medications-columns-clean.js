...
async function removeMedicationsColumns() {
  try {
    ...
    console.log('\n2️⃣ Cleaning up existing temp tables...');
    try {
      await runQuery('DROP TABLE IF EXISTS prescriptions_new');
      await runQuery('DROP TABLE IF EXISTS prescriptions_temp');
      console.log('✅ Temp tables cleaned up');
    } catch (e) {
      console.log('ℹ️  No temp tables to clean up');
    }
    
    // Disable foreign keys temporarily
    console.log('\n3️⃣ Temporarily disabling foreign keys...');
    await runQuery('PRAGMA foreign_keys = OFF');
    console.log('✅ Foreign keys disabled');
    
    // Create new prescriptions table without medications and labTestRecommendations
    console.log('\n4️⃣ Creating new prescriptions table structure...');
    ...
    console.log('\n5️⃣ Migrating data to new table...');
    await runQuery(`
